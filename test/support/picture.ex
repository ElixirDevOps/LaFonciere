defmodule Constat.Picture do
  ...
  it. The picture it writes is committed on the branch, because this repository's CI workflows
  are Constat's own generated files, which the runner's token is refused on: the branch is the
  only place the picture can reach a reader from.

  Test-support code, compiled only in `:test` — the product does not draw pictures.
